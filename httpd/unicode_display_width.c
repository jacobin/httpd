// https://github.com/p-ranav/unicode_display_width/blob/master/include/unicode/display_width.hpp#L376
...
int mk_wcswidth(const wchar_t* pwcs, size_t n)
{
    ...
    Bool_t in_escape_sequence = False;
    for (; *pwcs && n-- > 0; pwcs++) {
        if ((w = mk_wcwidth(*pwcs)) < 0) {
            ...
            if (in_escape_sequence) {
                if (iswalpha(*pwcs)) { // N.B. usually an 'm'
